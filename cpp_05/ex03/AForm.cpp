...
AForm::AForm(std::string _name, bool _is_signed, int _sign_grade, int _exe_grade) : name(_name) , is_signed(_is_signed), sign_grade(_sign_grade), exe_grade(_exe_grade)
{
...
AForm::AForm() : name("default") , is_signed(false), sign_grade(0), exe_grade(0) {}
...
int AForm::getExe_grade() const { return(exe_grade); }

std::ostream &operator<<(std::ostream &os, const AForm &Aform)
{
    os <<"NAME: " << Aform.getName() << std::endl;
    os <<"SIGNED: " << Aform.getIs_signed() << std::endl;
    os <<"SIGN GRADE: " << Aform.getSign_grade() << std::endl;
    os <<"EXE GRADE: " << Aform.getExe_grade() << std::endl;
    return (os);
}
